...
export const IconButton = ({
  ...
  return (
    <button
      className={`inline-flex items-center justify-center rounded-full border bg-indigo-950 text-xl leading-none text-white ${negative ? 'border-rose-900' : 'border-indigo-800'} ${auto ? 'px-3 py-1.5' : large ? 'h-12 w-12' : 'h-10 w-10'}`}
      {...props}
    >
      <span>{children}</span>
    </button>
